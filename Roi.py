...
class Roi:
    def __init__(self, canvas: tk.Canvas, x1, y1, x2, y2):
        ...
        # 기하학적 설정 (픽셀 단위)
        self.handle_radius = 5  # 검정색 원(핸들)의 반지름
        ...
        self.rect = self.canvas.create_rectangle(
            x1, y1, x2, y2,
            outline="goldenrod", width=2, dash=(6, 4),
            tags="roi_bound"
        )

        # B. 핑크색 중심 십자선 (Pink center cross)
        self.cross_h = self.canvas.create_line(0, 0, 0, 0, fill="hotpink", width=2, tags="roi_core")
        self.cross_v = self.canvas.create_line(0, 0, 0, 0, fill="hotpink", width=2, tags="roi_core")
        
        # C. 검정색 원 핸들 (Black handles: corners and midpoints)
    ...
    def on_handle_drag(self, event):
        ...
        min_size = 20
        if x2 - x1 < min_size:
            if "w" in h: x1 = x2 - min_size
            else: x2 = x1 + min_size
        if y2 - y1 < min_size:
            if "n" in h: y1 = y2 - min_size
            else: y2 = y1 + min_size
